...
if (isset($_POST['update'])) {
    ...
    if ($result) {
        ...
        if ($current_status === 'approved' && $status != 'canceled') {
            $_SESSION['message'] = [
                'text' => 'Cannot change status: Request is already approved. You can only cancel it.',
                'icon' => 'error'
            ];
        } else {
            // Update status
            ...
            if ($result) {
                if ($status == 'approved') {
                    $sql = "SELECT * FROM appointments WHERE request_id='$request_id'";
                    $result = mysqli_query($connect, $sql);
                    if ($result && mysqli_num_rows($result) > 0) {
                        // Already approved session message
                        $_SESSION['message'] = [
                            'text' => 'Request approved!',
                            'icon' =>'success'
                        ];
                    } else {
                        $sql = "INSERT INTO appointments (request_id) VALUES ('$request_id')";
                        $result = mysqli_query($connect, $sql);
                        if ($result) {
                            $_SESSION['message'] = [
                                'text' => 'Request updated and Appointment booked successfully!',
                                'icon' => 'success'
                            ];
                        } else {
                            $_SESSION['message'] = [
    ...
    header('Location: admin_appointment_requests.php?id=' . $id);
    exit();
}
...
<body>
    ...
    <div class="sidebar">
            <ul>
                ...
                <li class="active" ><a  class="active" href="admin_appointment_requests.php" class="nav_links"> appointment requests</a></li>
        ...
        <div class="main_content">
            ...
            <div>
                <div class="container">
                    <h1 class="form-title">update request status</h1>
                    <form method="post">
