...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM vaccination_reminders WHERE id ='$id'";
...
if (isset($_POST['update'])) {
   ...
   $sql = "UPDATE vaccination_reminders SET status='$status' WHERE id = '$id'";
   $result = mysqli_query($connect, $sql);

    if ($result) {
        $_SESSION['message'] = [
            'text' => 'Vaccination reminder status updated successfully!',
            'icon' =>'success'
        ];
        ...
        $_SESSION['message'] = [
            'text' => 'Failed to update vaccination reminder status!',
            'icon' => 'error'
        ];
        echo "Error updating record: ". mysqli_error($connect);
       
    }
    header('Location: admin_reminder.php?id=' . $id);
    exit();
}
