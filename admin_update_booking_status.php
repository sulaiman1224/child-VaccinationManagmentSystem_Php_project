...
if (isset($_POST['update'])) {
    ...
    if ($result) {
        ...
        if ($current_status === 'booked' && $status != 'canceled') {
            $_SESSION['message'] = [
                'text' => 'Cannot change status: Request is already booked. You can only cancel it.',
                'icon' => 'error'
            ];
        } else {
            // Update status
            $sql = "UPDATE appointments SET status='$status' WHERE id = '$booking_id'";
            $result = mysqli_query($connect, $sql);

            if ($result) {
                if ($status == 'booked') {
                    $sql = "SELECT * FROM vaccination_reports WHERE booking_id='$booking_id'";
                    $result = mysqli_query($connect, $sql);
                    if ($result && mysqli_num_rows($result) > 0) {
                        ...
                        $sql = "INSERT INTO vaccination_reports (booking_id) VALUES ('$booking_id')";
                        $result = mysqli_query($connect, $sql);
                        if ($result) {
                            $_SESSION['message'] = [
                                'text' => 'Status updated and Appointment booked successfully!',
                                'icon' => 'success'
                            ];
                        } else {
                            $_SESSION['message'] = [
    ...
    header('Location: admin_booking.php?id=' . $id);
    exit();
}
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="main_content">
            ...
            <div>
                <div class="container">
                    <h1 class="form-title">update booking status</h1>
                    <form method="post">
